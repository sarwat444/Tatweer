...
class UsersController extends Controller
{
    ...
    public function verified_student($id = '')
    {
        $query = User::where('id', $id)->first();
            if ($query->verified_student == 1) {
                $query->update(['verified_student' => 0]);
                $response = [
                    'success' => get_phrase('updated successfully')
                ];
            return json_encode($response);
        }else{
        
            $query->update(['verified_student' => 1]);
            $response = [
                'success' => get_phrase('updated successfully')
            ];
        return json_encode($response);
