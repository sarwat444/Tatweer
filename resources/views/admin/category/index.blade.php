...
@endpush
@push('css')
    <link rel="stylesheet" type="text/css" href="{{ asset('assets/backend/css/categories.css') }}">
@endpush
@section('content')
        ...
        <div class="ol-card-body my-3 py-12px px-20px">
            <div class="d-flex align-items-center justify-content-between gap-3 flex-wrap flex-md-nowrap">
                <h4 class="title fs-16px">
                    ...
                    {{ get_phrase('All Grade') }} <span class="text-muted">({{ $categories->count() }})</span>
                </h4>

                <a onclick="ajaxModal('{{ route('modal', ['admin.category.create', 'parent_id' => 0]) }}', '{{ get_phrase('Add new grade') }}')" href="#" class="btn ol-btn-outline-secondary d-flex align-items-center cg-10px">
                    <span class="fi-rr-plus"></span>
                    <span>{{ get_phrase('Add new grade') }}</span>
                </a>
            </div>
        </div>
    </div>
