...
<div class="row mb-3">
    <label for="category_id" class="form-label ol-form-label col-sm-2 col-form-label">{{ get_phrase('Grade') }}</label>
    <div class="col-sm-8">
        <select name="category_id" id="category_id" class="form-control ol-form-control">
            @foreach ($categories as $category)
            <option @isset($student->category_id) @selected($student->category_id==$category->id) @endisset value="{{$category->id}}">{{$category->title}} </option> 
            @endforeach
        </select>
    </div>
</div>
